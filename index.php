...
function handleSQLException($message) {
    echo "<script type='text/javascript'>
        alert('$message');
        window.location.href = 'index.php';
    </script>";
    exit(); // Terminate script execution after handling exception
}

// Stored procedure to insert user with handling for mobile number length
// Drop the procedure if it exists
$query = "DROP PROCEDURE IF EXISTS Insert_User";
if (!mysqli_query($connection, $query)) {
    handleSQLException('Error dropping existing procedure.');
}
...
BEGIN
    DECLARE mobile_length INT;
    SET mobile_length = LENGTH(mobile);
    
    IF mobile_length > 10 THEN
        SET mobile = '0000000000';
    END IF;

    INSERT INTO users (fname, lname, email, password, mobile, address)
    VALUES (fname, lname, email, password, mobile, address);
END
";
if (!mysqli_query($connection, $query)) {
    handleSQLException('Error creating stored procedure.');
}
...
if (isset($_POST['register'])) {
    // Call the stored procedure instead of directly executing the query
    $fname = $_POST['fname'];
    $lname = $_POST['lname'];
    $email = $_POST['email'];
    ...
    $query = "CALL Insert_User('$fname', '$lname', '$email', '$password', '$mobile', '$address')";
    $query_run = mysqli_query($connection, $query);

    if (!$query_run) {
        handleSQLException('Registration failed... Please try again.');
    }
...
<html lang="en" dir="ltr">
  ...
  <body style="background:url('images/mess-img3.jpeg');">
    <!-- Navbar -->
